...
async function main() {
  ...
  const civs = heatmapDropdown(data)
  heatmap(data, civs[0], civs[0])
}

function heatmapDropdown(data: Unit[]) {
  ...
  document.getElementById("alliedCivSelectButton").appendChild(labelAllied).appendChild(selectAllied);
  document.getElementById("rivalCivSelectButton").appendChild(labelRival).appendChild(selectRival);
  ...
  for (const civ of civs)
  {
    ...
    option.value = civ;
    option.text = civTitles[civ].charAt(0).toUpperCase() + civTitles[civ].slice(1);
  ...
  document.getElementById("rivalCivSelectButton").onchange = function() {
    ...
    heatmap(data, selectAllied.value, selectRival.value)
  }
...
function getModifierHeatmapData(alliedUnits: Unit[], rivalUnits: Unit[]): [number, any[]] {
  var heatmapData: any[] = [];
  ...
  alliedUnits.forEach(alliedUnit => {
    ...
    let alliedRivalPairs = rivalUnits.map(rivalUnit => {
      var obj = undefined
...
function heatmap(data: Unit[], allyCiv: string, rivalCiv: string) {
  ...
  const modifierHeatmapData = getModifierHeatmapData(uniqueAlliedUnits, uniqueRivalUnits)
  const maxBonus = modifierHeatmapData[0]
  const heatmapData = modifierHeatmapData[1]

  // Build color scale
  var myColor = d3.scalePow<string>()
      .exponent(0.4)
      .domain([0, maxBonus])
      .range(["white", "#074b70"])

  // Create a tooltip
  var tooltip = d3.select("#unitHeatmap")
      .append("div")
      .style("opacity", 0)
      .attr("class", "tooltip")
  ...
  var mouseover =  (event: any, de: any) => {
    ...
    svg.selectAll()
        .data(heatmapData, function(d) {return d.uX+':'+d.uY;})
        .enter().append("rect")
        .attr("id", "high2")
  ...
  var mousemove = (event: any, d: any) => {
    tooltip
        .html("<strong style = 'color:green'>" + d.uY + "</strong> has <br> <strong style = 'color:blue'>" + d.uV + "</strong> bonus attack against <br> <strong style = 'color:red'>" + d.uX + "</strong>")
  ...
  svg.selectAll()
      ...
      .enter()
      .append("rect")
      .attr("x", function(d) { return x(d.uX) })
      .attr("y", function(d) { return y(d.uY) })
      .attr("width", x.bandwidth() )
      .attr("height", y.bandwidth() )
      .style("fill", function(d) { return myColor(d.uV)} )
      .style("stroke", "#c7c7c7")
